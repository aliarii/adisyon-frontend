...
const Sidebar = ({ openSideBar, setSideBar }) => {
  ...
  useEffect(() => {
    ...
    const firstSection = pathSegments[2];
  ...
  const handleNavigate = (item) => {
    ...
    navigate(`/adisyon-frontend/${item.path}`);
  };
  ...
  return (
    <div
      className={`fixed flex flex-col w-64 h-full px-2.5 pt-5 pb-3 z-[92] top-0 left-0 gap-5 overflow-x-hidden ${
        openSideBar ? "visible" : "hidden"
      } bg-light-1 dark:bg-dark-10 text-dark-7 dark:text-light-7`}
    >
      <div className="flex items-center justify-between w-full min-h-[60px] px-2 bg-light-2 dark:bg-dark-8 rounded-xl text-center ">
        <h1
          className={`cursor-pointer font-medium text-xl overflow-hidden text-ellipsis ${
            !openSideBar && "scale-0"
          }`}
          onClick={() => handleNavigate(sidebarItems[0])}
        >
      ...
      <div className="flex justify-between items-center w-full min-h-[60px] p-2 rounded-xl bg-light-2 dark:bg-dark-8 text-lg font-medium">
        <div
          ...
          onClick={() => handleNavigate({ path: "settings/profile" })}
        >
          <AccountCircleIcon />
          <h2 className="uppercase">{user ? user.fullName : "Loading..."}</h2>
        </div>
        <div
          className="flex items-center p-2 rounded-full cursor-pointer hover:bg-light-4 hover:dark:bg-light-5 hover:dark:text-dark-7"
          onClick={logout}
        >
          <LogoutIcon />
        </div>
