    ...
    return (
        <div className='flex justify-center items-center w-screen h-screen bg-white dark:bg-dark-7'>
            <div className='flex flex-col w-80 md:w-96 m-auto p-4 rounded-xl bg-light-3 dark:bg-dark-5 gap-4'>
                <h1 className='text-dark-7 dark:text-light-8 text-4xl font-semibold '>{t("Register")}</h1>
                ...
                <form className='flex flex-col gap-4' onSubmit={handleSubmit}>
                    ...
                    {getFieldErrors("Company Name").map((error, index) => (
                        <h2 key={index} className="-mt-4 text-sm text-red-500">*{t(error)}</h2>
                    ))}
                    <input
                        type='text'
                        autoCapitalize='off'
                    ...
                    {errorMessage && <h2 className='-mt-4 text-sm text-red-500'>*{errorMessage}</h2>}
                ...
                <a className=' text-center cursor-pointer text-blue-700 dark:text-blue-500 font-bold no-underline text-sm' href='/login'>{t("Back to Login")}</a>
            </div>
        </div>
    );
};
