...
import { deleteOrder, getOrderByBasketId, getOrderById, payAllOrders, payAmount, paySelectedOrders, transferOrders, updateOrder } from '../../State/Order/Action';
...
const OrdersView = () => {
    ...
    const windows = [
        (props) => <PayOrdersWindow handleWindowSelect={handleWindowSelect} {...props} />,
        (props) => <PaySelectedOrdersWindow handleWindowSelect={handleWindowSelect} {...props} />,
        (props) => <PayAmountWindow handleWindowSelect={handleWindowSelect} {...props} />,
        (props) => <TransferOrdersWindow handleWindowSelect={handleWindowSelect} {...props} />,
        (props) => <EditOrdersWindow handleWindowSelect={handleWindowSelect} pendingOrders={pendingOrders} {...props} />,
    ];
    ...
    useEffect(() => {
        if (basket)
            dispatch(getOrderByBasketId(basket.id));

    }, [basket, dispatch])

    useEffect(() => {
        const pendingItems = order?.orderItems?.filter(item => item.status === "STATUS_PENDING") || [];
    ...
    return (
        <div className='flex flex-col h-full gap-1 p-1 pt-0 overflow-auto'>
            ...
            <div className={`flex flex-col h-5/6 w-full overflow-auto relative ${selectedPaymentWindow ? 'overflow-hidden' : ''}`}>
                {showPaymentWindow && (
                    <>
                ...
                <div className='flex flex-col gap-1 size-full '>
                    {pendingOrders.length > 0 ?
                        pendingOrders.map((item, index) => (
                            <div key={index}>
                                <OrderItem item={item} handleWindowSelect={handleWindowSelect} />
            ...
            {pendingOrders.length > 0 &&
                <div className='flex flex-col gap-1'>
                    ...
                    <div className='flex justify-between text-end'>
                        <h2>{t("Total Price")}:</h2>
                        <h2>{t("$")}{order?.totalPrice}</h2>
                    </div>
                    {order?.payments?.length > 0 &&
                        <div className='flex justify-between text-end text-sm'>
                            <h2>{t("Payment History")}:</h2>
                            <h2>-{t("$")}{order?.payments?.reduce((total, item) => total + item.payAmount, 0).toFixed(2)}</h2>
                        </div>
                    }
            ...
            {pendingOrders.length > 0 ?
                <div className='h-fit bg-light-6 dark:bg-dark-3 rounded-lg'>
                    <OrdersController handleWindowSelect={handleWindowSelect} pendingOrders={pendingOrders} />
                </div> : ""
            }
        </div>
    )
    ...
    return (
        <div className="flex items-center justify-between p-1 size-full">
            ...
            <div className='flex w-3/6 justify-end gap-2'>
                <h2>{t("$")}{item.product.price * item.quantity}</h2>
                <EditIcon className='self-center cursor-pointer' onClick={() => handleEditClick()} />
            </div>
...
const OrdersController = ({ handleWindowSelect, pendingOrders }) => {
    ...
    const handlePaySelectedOrders = () => handleWindowSelect(1, { pendingOrders });
    ...
    const handleCancelOrders = () => {
        ...
        dispatch(createRecordItem(recordReqData))
            .then(() => { return dispatch(deleteOrder(reqData)) })
            .then(() => { return dispatch(getCompanyByUserId(auth.user, jwt)) })
    ...
    return (
        <div className="flex flex-col size-full p-2 items-center justify-end gap-2 text-center text-light-1">
            <div className='flex flex-row size-full items-center justify-center gap-2'>
                <div className='w-full bg-success p-1 rounded-lg cursor-pointer' onClick={handlePayOrders}>
                    <h2>{t("Pay All")}</h2>
                </div>
                <div className='w-full bg-success p-1 rounded-lg cursor-pointer' onClick={handlePaySelectedOrders}>
                    <h2>{t("Pay Selectively")}</h2>
                </div>
                <div className='w-full bg-success p-1 rounded-lg cursor-pointer' onClick={handlePayAmount}>
                    <h2>{t("Pay Amount")}</h2>
                </div>
                {/* <div className='bg-red-500 p-1 rounded-lg cursor-pointer' onClick={handleCloseBasket}>Cancel</div> */}
            </div>
            <div className='flex flex-row size-full items-center justify-center gap-2'>
                <div className='w-full bg-info p-1 rounded-lg cursor-pointer' onClick={handleTransferOrders}>
                    <h2>{t("Transfer")}</h2>
                </div>
                <div className='w-full bg-danger p-1 rounded-lg cursor-pointer' onClick={handleCancelOrders}>
...
const EditOrdersWindow = ({ handleWindowSelect, pendingOrders }) => {
    ...
    const updateOrderItem = (newOrderItem) => {
        const updatedOrders = copyOrders.map(item => item.id === newOrderItem.id ? ({
            ...item,
    ...
    const handleSave = () => {
        ...
        dispatch(createRecordItem(recordReqData))
            .then(() => { return dispatch(updateOrder(reqData)) })
            .then(() => { return dispatch(getOrderById(jwt, orderId)); })
            ...
            <div className='flex flex-col size-full gap-2 overflow-auto'>
                ...
                <div className='w-full h-full px-2 flex flex-col gap-1 overflow-auto'>
                    {pendingOrders.map((item) => (
                        <EditOrderItem order={item} key={item.id} orderItem={item} updateOrderItem={updateOrderItem} />
                    ))}
        ...
        <div className="flex items-center justify-between px-2 py-1 rounded-lg bg-light-6 dark:bg-dark-2">
            ...
            <div className='w-3/6 flex items-center justify-end gap-2'>
                <div className='w-24 flex justify-center items-center gap-1'>
                    <AddCircleIcon className='cursor-pointer' onClick={() => { updateTempOrderQuantity(1) }} />
                    <h2 className='w-6 text-center'>{tempOrderItem.quantity}</h2>
                    <RemoveCircleIcon className='cursor-pointer' onClick={() => { updateTempOrderQuantity(-1) }} />

                </div>
                <h2 className='min-w-12'>{t("$")}{tempOrderItem.quantity * orgOrderItem.product.price}</h2>
                <CancelIcon className='cursor-pointer' onClick={() => { updateTempOrderQuantity(-tempOrderItem.quantity) }} />
            </div>
        </div>
    );
};
...
const PayOrdersWindow = ({ handleWindowSelect, pendingOrders }) => {
    ...
    const handlePayAllOrders = (type) => {
        ...
        dispatch(createRecordItem(recordReqData))
            .then(() => { return dispatch(payAllOrders(reqData)) })
            .then(() => { return dispatch(getCompanyByUserId(auth.user, jwt)) })
            .then(() => { navigate(`/baskets`); })
    }
    ...
    return (
        <div className='flex flex-col size-full p-2 overflow-auto'>
            ...
            <div className='flex flex-col size-full gap-2 overflow-auto'>
                ...
                <div className='w-full h-full px-2  flex flex-col gap-1 overflow-auto'>
                    {pendingOrders.map((item) => (
                        <div key={item.id} className="flex items-center justify-between px-2 py-1 rounded-lg bg-light-6 dark:bg-dark-2">
                            <div className='flex h-full w-3/6 justify-start gap-2'>
            ...
            <h2 className='text-end py-1 px-2'>{t("Total Price")}: {t("$")}{basket.order.totalPrice.toFixed(2)}</h2>
            <div className='h-fit p-2 text-light-1'>
                <div className='flex gap-2'>
                    <button className='w-full mb-2 py-1 rounded-lg bg-success' onClick={() => handlePayAllOrders("Cash")}>
                        <h2>{t("Cash")}</h2>
                    </button>
                    <button className='w-full mb-2 py-1 rounded-lg bg-success' onClick={() => handlePayAllOrders("CreditCard")}>
                        <h2>{t("Credit Card")}</h2>
                    </button>
                </div>
                <button className='w-full py-1 rounded-lg bg-danger' onClick={() => handleWindowSelect(null)}>
                    <h2>{t("Cancel")}</h2>
                </button>
            </div>
        </div>
...
const PaySelectedOrdersWindow = ({ handleWindowSelect, pendingOrders }) => {
    ...
    const handlePaySelectedOrders = (type) => {
        ...
        dispatch(createRecordItem(recordReqData))
            .then(() => { return dispatch(paySelectedOrders(reqData)) })
            .then(() => handleWindowSelect(null))
            .then(() => { return dispatch(getCompanyByUserId(auth.user, jwt)); })
            .then(() => { return dispatch(getOrderById(jwt, order.id)); })
    }

    const calculateTotalPrice = () => {
        const total = copyOrders?.reduce((total, item) => total + (item.quantity * item.product.price), 0);
    ...
    return (
        <div className='flex flex-col size-full p-2 overflow-auto'>
            ...
            <div className='flex flex-col size-full gap-2 overflow-auto'>
                ...
                <div className='w-full h-full px-2  flex flex-col gap-1 overflow-auto'>
                    {pendingOrders.map((item) => (
                        <PaySelectedOrderItem key={item.id} orderItem={item} updateOrderItem={updateOrderItem} />
                    ))}
                </div>
            </div>
            <h2 className='text-end py-1 px-2'>{t("Total Price")}: {t("$")}{calculateTotalPrice().toFixed(2)}</h2>
            <div className='h-fit p-2 text-light-1'>
                <div className='flex gap-2'>
                    <button className='w-full mb-2 py-1 rounded-lg bg-success' onClick={() => handlePaySelectedOrders("Cash")}>
                        <h2>{t("Cash")}</h2>
                    </button>
                    <button className='w-full mb-2 py-1 rounded-lg bg-success' onClick={() => handlePaySelectedOrders("CreditCard")}>
                        <h2>{t("Credit Card")}</h2>
                    </button>
                </div>
                <button className='w-full py-1 rounded-lg bg-danger' onClick={() => handleWindowSelect(null)}>
...
const PaySelectedOrderItem = ({ orderItem, updateOrderItem }) => {
    const [orgOrderItem] = useState(orderItem);
    const [tempOrderItem, setTempOrderItem] = useState({ ...orderItem, quantity: 0 });
    const { t } = useTranslation();
...
const PayAmountWindow = ({ handleWindowSelect }) => {
    ...
    const handlePay = (type) => {
        if (enteredQuantity === 0 || enteredQuantity < 0 || enteredQuantity > order.totalPrice) {
        ...
        <div className='flex flex-col size-full p-2 overflow-auto'>
            ...
            <div className='flex flex-col size-full gap-2 overflow-auto'>
                ...
                <div className='flex flex-col items-center justify-center w-full h-full px-2 overflow-auto'>
                    ...
                    />
                    {errorMessage && <h2 className='h-fit p-2 text-red-500'>*{t(errorMessage)}</h2>}

                </div>
            </div>
            <div className='h-fit p-2 text-light-1'>
                <div className='flex gap-2'>
                    <button className='w-full mb-2 py-1 rounded-lg bg-success' onClick={() => handlePay("Cash")}>
                        <h2>{t("Cash")}</h2>
                    </button>
                    <button className='w-full mb-2 py-1 rounded-lg bg-success' onClick={() => handlePay("CreditCard")}>
                        <h2>{t("Credit Card")}</h2>
                    </button>
                </div>
                <button className='w-full py-1 rounded-lg bg-danger' onClick={() => handleWindowSelect(null)}>
...
const TransferOrdersWindow = ({ handleWindowSelect, pendingOrders }) => {
    ...
    const handleCompleteTransfer = () => {
        ...
        dispatch(createRecordItem(recordReqData))
            .then(() => { return dispatch(transferOrders(reqData)) })
            .then(() => { return dispatch(getCompanyById(company.id, jwt)) })
            .then(() => { navigate(`/baskets`); })
            .then(() => { navigate(`/basket/${selectedBasket.name}`, { state: selectedBasket }) })
            .then(() => handleWindowSelect(null))
    }
    ...
    const windows = [
        (<div className='flex flex-col size-full gap-2 overflow-auto'>
            ...
            <div className='w-full h-full px-2 flex flex-col gap-1 overflow-auto'>
                {pendingOrders.map((item) => (
                    <TransferOrderItem order={item} key={item.id} orderItem={item} updateOrderItem={updateOrderItem} />
                ))}
            </div>
            <div className='mx-2 flex justify-end gap-2 text-light-1'>
                <button disabled={true} className='px-3 py-1 rounded-lg bg-success disabled:bg-dark-4 disabled:text-light-10'>
                    <h2>{t("Prev")}</h2>
                </button>
                <button className='px-3 py-1 rounded-lg bg-success' onClick={() => setCurWindow(1)}>
                    <h2>{t("Next")}</h2>
                </button>
            </div>
        </div >),
        (<div className='flex flex-col h-full gap-2 overflow-auto'>
            ...
            <div className='w-full h-full px-2 overflow-auto'>
                <Grid2 container spacing={0.5} columns={{ xs: 4, sm: 8, md: 12, lg: 16 }} >
                    {/* {company ? company.baskets.filter((basket) => !basket.isActive).map((item, index) => ( */}
                    {company ? company.baskets.map((item, index) => (
                        <Grid2 key={index} size={{ xs: 2, sm: 4, md: 3, lg: 4 }}>
                            <div className={`min-h-[82px] rounded-md cursor-pointer text-center bg-dark-3 hover:bg-white hover:dark:bg-dark-3 ${selectedBasket === item ? "border-2" : ""}`} onClick={() => handleBasketSelect(item)}>
            ...
            <div className='mx-2 flex justify-end gap-2 text-light-1'>
                <button className='px-3 py-1 rounded-lg bg-success' onClick={() => setCurWindow(0)}>
                    <h2>{t("Prev")}</h2>
                </button>
                <button disabled={selectedBasket ? false : true} className='px-3 py-1 rounded-lg bg-success disabled:bg-dark-4 disabled:text-light-10' onClick={() => setCurWindow(2)}>
                    <h2>{t("Next")}</h2>
                </button>
        ...
        (<div className='flex flex-col w-full h-full gap-2 overflow-auto'>
            ...
            <div className='size-full flex flex-col px-2 gap-1 overflow-auto'>
                {copyOrders.map((order, idx) => (
                    <div key={idx} className="flex items-center justify-between px-2 py-1 rounded-lg bg-light-6 dark:bg-dark-2">
                        <div className='w-3/6 flex items-center justify-start'>
                        ...
                        <div className='w-3/6 flex items-center justify-end gap-2'>
                            ...
                            <h2 className='min-w-12'>{t("$")}{order.quantity * order.product.price}</h2>
                        </div>
                    </div>
                ))}
            ...
            <div className='flex flex-col justify-center items-center h-fit mx-2 p-3 gap-1 bg-light-6 dark:bg-dark-3 rounded-lg'>
                <h2>{basket?.name} {"->"} {selectedBasket?.name}.</h2>
            ...
            <div className='mx-2 flex justify-end gap-2 text-light-1'>
                ...
                <button disabled={selectedBasket ? false : true} className='px-3 py-1 rounded-lg bg-success disabled:bg-dark-4 disabled:text-light-10' onClick={() => handleCompleteTransfer()}>
                    <h2>{t("Transfer")}</h2>
                </button>
            </div>
        </div>)
    ...
    return (
        <div className='flex flex-col size-full p-2 overflow-auto'>
            ...
            <div className='h-fit p-2'>
                <button className='w-full py-1 rounded-lg bg-danger text-light-1' onClick={() => handleWindowSelect(null)}>Cancel</button>
            </div>
...
const SmallScreenPayWindow = ({ onClose, children }) => {
    return (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-40">
            <div className='flex items-center justify-center size-full text-center cursor-default' onClick={onClose}>

                {/* onClick={(e) => e.stopPropagation()} to prevent the close function execution */}
                <div className='bg-dark-5 rounded-xl h-3/5 w-3/4' onClick={(e) => e.stopPropagation()}>
                    {children}
                </div>
            </div>
        </div>
