...
const BasketDetailsPage = () => {
    ...
    const views = [
        ...
        { id: 2, title: "Waiting Orders", element: (props) => <WaitingOrdersView  {...props} handleViewSelect={() => handleViewSelect(views[0])} /> },

        { id: 3, title: "Payment History", element: (props) => <PaymentHistoryView {...props} /> },
    ...
    return (
        <div className='flex flex-col h-full gap-2'>
            ...
            <div className="flex items-center justify-start z-[90] w-full p-2 gap-3 rounded-xl bg-light-2 dark:bg-dark-7 text-dark-7 dark:text-light-7">
                ...
                <ArrowBackIcon className={`p-2.5 min-h-11 min-w-11 rounded-lg cursor-pointer bg-white dark:bg-dark-5 hover:bg-light-4 hover:dark:bg-light-5 hover:dark:text-dark-7`}
                    onClick={() => navigate("/baskets")} />
                <h2 className="text-center whitespace-nowrap text-lg font-medium">{getBasketDisplayName()}</h2>
            ...
            <div className='flex flex-col md:flex-row h-full w-full pb-3 gap-2 font-semibold text-lg text-dark-7 dark:text-light-7 overflow-auto'>
                ...
                <div className='min-h-12 md:h-full w-full md:w-2/12 overflow-auto'>
                    <ProductCategoriesView categories={categories} selectedCategory={selectedCategory} handleCategorySelect={handleCategorySelect} />
                </div>


                <div className='h-full md:h-full w-full md:w-7/12 overflow-auto'>
                    <ProductsView products={searchItem !== "" ? filteredProducts : selectedCategory?.products} searchItem={searchItem} handleSearchInputChange={handleSearchInputChange} handleViewSelect={() => handleViewSelect(views[1])} />
                </div>
                    ...
                    <div className='flex flex-col w-full h-full rounded-xl bg-light-2 dark:bg-dark-7 text-dark-7 dark:text-light-7 text-sm md:text-lg overflow-auto'>
                        ...
                        <div className='flex justify-between items-center w-full min-h-11 px-2 py-2 overflow-auto'>
                            {views.map((view, index) => (
                                <h2 className={`w-full text-center ${selectedView.title === view.title ? "bg-light-5 dark:bg-dark-5 rounded-lg" : ""} py-1 px-2 whitespace-nowrap text-sm cursor-pointer`} key={index} onClick={() => handleViewSelect(view)}>{t(view.title)}</h2>
                            ))}
                        </div>
