...
const settingsPages = [
    { name: "Company Settings", element: <CompanySettingsPage />, path: "company" },
    { name: "Product Settings", element: <ProductSettingsPage />, path: "product" },
    { name: "Basket Settings", element: <BasketSettingsPage />, path: "basket" },
    { name: "User Settings", element: <UserSettingsPage />, path: "user" },
    { name: "Profile Settings", element: <ProfileSettingsPage />, path: "profile" },
]
...
const SettingsPage = ({ settingsIndex }) => {
    ...
    const [selectedSettingsPage, setSelectedSettingsPage] = useState(settingsPages[0]);
            ...
            <div className='flex flex-col w-full md:w-[calc(20%)] min-w-36 p-2 bg-light-2 dark:bg-dark-7 rounded-lg overflow-auto '>
                ...
                <div className='flex flex-row md:flex-col h-full'>
                    {settingsPages.map((item, index) => (
                        <div className={`flex justify-between items-center py-2 cursor-pointer rounded-lg ${selectedSettingsPage?.name === item.name ? "bg-light-4 dark:bg-dark-5" : ""}`} key={index} onClick={() => handleNavigate(item)}>
                            <h2 className='mr-2 md:mr-0 ml-2 whitespace-nowrap overflow-hidden text-ellipsis'>{t(item.name)}</h2>
