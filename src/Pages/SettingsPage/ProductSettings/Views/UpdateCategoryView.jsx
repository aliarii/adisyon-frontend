...
import { deleteProductCategory, updateProductCategory } from '../../../../State/ProductCategory/Action';
import BtnMdCancel from '../../../../Components/Button/BtnMdCancel';
import { useTranslation } from 'react-i18next';



const UpdateCategoryView = ({ id, name, products, handleCancel }) => {
    const { company } = useSelector((store) => store.company)
    const dispatch = useDispatch();
    const [categoryName, setCategoryName] = useState(name);
    const [notSelectedProducts, setNotSelectedProducts] = useState(
        company?.products?.filter(
            (companyProduct) => !products.some((selectedProduct) => selectedProduct.id === companyProduct.id)
        ) || []
    );
    ...
    const handleCategoryDelete = () => {
        ...
        console.log("reqData", reqData);
        dispatch(deleteProductCategory(reqData))
            .then(() => dispatch(getCompanyById(company.id, localStorage.getItem('jwt'))))
    }
